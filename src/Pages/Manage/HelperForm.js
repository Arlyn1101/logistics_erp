...
import {
  ...
  deleteHelperAttachment,
  downloadHelperAttachment,
...
export default function HelperForm() {
  ...
  const passed_helper = location.state?.helper || null;
  const is_edit = !!passed_helper;
  ...
  async function fetch_attachments(helper_id) {
    const response = await getHelperAttachments(helper_id);
    if (response.data) {
  ...
  async function load_helper() {
    if (!is_edit) return;
    const response = await getHelperDetails(passed_helper.id);
  ...
  useEffect(() => {
    load_helper();
    if (is_edit) fetch_attachments(passed_helper.id);
  }, []);
  ...
  async function handle_save() {
    if (!validateHelper(form, set_is_error)) return;
    set_is_clicked(true);
    const response = is_edit
      ? await updateHelper(form, attachments)
      : await createHelper(form, attachments);
    if (response.data && response.data.response) {
      toast.success(
        is_edit ? "Helper updated successfully!" : "Helper added successfully!",
        { style: toastStyle() }
      );
      navigate("/helpers");
    } else {
      toast.error(
        is_edit ? "Failed to update helper." : "Failed to add helper.",
        { style: toastStyle() }
      );
    }
    ...
    <div>
      ...
      <div className={`manager-container ${inactive ? "inactive" : "active"}`}>
        ...
        <div className="add-customer-breadcrumb">
          <span className="breadcrumb-link" onClick={() => navigate("/helpers")}>
            Helpers
          </span>
          <span className="breadcrumb-sep">›</span>
          <span className="breadcrumb-current">
            {is_edit ? "Edit Helper" : "Add New Helper"}
          </span>
        </div>
        ...
        <div className="add-customer-header">
          <div>
            ...
            <p className="page-subtitle">
              {is_edit
                ? "Update helper personnel information"
                : "Fill in helper personnel information"}
            </p>
          </div>
        ...
        <div className="biodata-card">
          ...
          {/* ── Section 1: Helper Information ── */}
          <div className="biodata-section-label">Helper Information</div>
        ...
        {/* ── Section 5: Documents ── */}
          ...
          {is_edit && saved_attachments.length > 0 && (
            <div className="mt-2">
              <div className="field-label">ALREADY UPLOADED:</div>
                ...
                <div className="attachment-row" key={att.id}>
                  ...
                  <div className="attachment-actions">
                    <button
                      ...
                      type="button"
                      onClick={() => downloadHelperAttachment(att.file_path, att.file_name)}
                    >
                    ...
                    <button
                      ...
                      type="button"
                      onClick={async () => {
                        await deleteHelperAttachment(att.id);
                        fetch_attachments(form.id);
                      }}
                    >
                      <FontAwesomeIcon icon={faTrash} />
                    </button>
