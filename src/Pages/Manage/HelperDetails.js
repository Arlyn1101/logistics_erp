...
export default function HelperDetails() {
  ...
  async function fetch_attachments() {
    const response = await getHelperAttachments(id);
    if (response.data && response.data.data) {
  ...
  async function handle_save() {
    ...
    if (pending_delete_id) {
      await deleteHelperAttachment(pending_delete_id);
      set_pending_delete_id(null);
    }

    const attachments_to_upload = new_attachment ? [new_attachment] : [];
    const response = await updateHelper(form, attachments_to_upload);
  ...
  if (is_loading) {
    return (
      <div>
        <div className="page">
          <Navbar onCollapse={(v) => set_inactive(v)} active={"HELPERS"} />
        </div>
        <div
          className={`manager-container ${inactive ? "inactive" : "active"}`}
        >
          <div className="details-loading">Loading helper details…</div>
        </div>
      </div>
    );
  }
      ...
      <div className={`manager-container ${inactive ? "inactive" : "active"}`}>
        <div className="add-customer-breadcrumb">
          ...
          <span className="breadcrumb-current">
            {full_name || "Helper Details"}
          </span>
        </div>

        <div className="add-customer-header">
          <div>
            <h1 className="page-title">{full_name || "Helper Details"}</h1>
            <p className="page-subtitle">
              {is_edit_mode
                ? "Edit helper information below"
                : "Viewing helper information"}
            </p>
          </div>
            ...
            <div key={att.id} className="attachment-row">
              ...
              <div className="attachment-actions">
                <button
                  ...
                  title="Download"
                  onClick={() =>
                    downloadHelperAttachment(att.file_path, att.file_name)
                  }
                  type="button"
                >
