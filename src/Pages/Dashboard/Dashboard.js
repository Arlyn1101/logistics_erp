...
export default function Dashboard() {
  ...
  const [month_stats, set_month_stats] = useState({
    total: "—",
    excess: "—",
    active_contracts: "—",
  });
  ...
  async function fetch_counts() {
    ...
    console.log("trips_res:", trips_res);
    console.log("trips sample:", trips_res.data.data[0]);

    const active_trucks =
      trucks_res.data?.data?.filter((t) => t.status === "active").length ?? "—";
    const active_drivers =
      drivers_res.data?.data?.filter((d) => d.status === "active").length ??
      "—";
    const active_helpers =
      helpers_res.data?.data?.filter((h) => h.status === "active").length ??
      "—";
    const total_customers = customers_res.data?.data?.length ?? "—";
    const active_contracts =
      contracts_res.data?.data?.filter((c) => c.status === "active").length ??
      "—";
    ...
    const excess_this_month = trips_this_month.filter(
      (t) => t.is_excess == 1,
    ).length;
    set_month_stats({
              ...
              <div className="stats-body">
                <div className="stats-rows">
                  {[
                    {
                      label: "Active Contracts",
                      value: month_stats.active_contracts,
                    },
              ...
              <div className="donut-body">
                <PieChart width={160} height={160}>
                  <Pie
                    data={
                      donut_total === 0
                        ? [{ name: "No data", value: 1, color: "#edf0f4" }]
                        : donut_data
                    }
                    cx={75}
                    cy={75}
                  ...
                  recent_trips.map((trip, idx) => (
                    ...
                    >
                      ...
                      <div className="trip-card-footer">
                        <span className="trip-card-customer">
                          {trip.customer_name || "—"}
                        </span>
                ...
                {contract_alerts.length === 0 ? (
                  ...
                  contract_alerts.map((c, idx) => (
                    <div className="alert-row" key={idx}>
                      ...
                      <span
                        className={`alert-dot ${c.days_remaining <= 0 ? "dot-red" : "dot-orange"}`}
                      />
                    </div>
                  ))
                )}
